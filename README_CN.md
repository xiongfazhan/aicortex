...
**AICortex** 是一个强大的 LLM 命令行工具，作为统一?AI 智能中枢，支持与多个 AI 提供商交互。它结合了命令行界面的简洁性和强大?AI 功能，如 RAG、Agent 和函数调用?

### 为什么选择 AICortex?

AICortex 为所?AI 交互提供统一的中心枢纽：
- **统一接口** - 一个工具支?OpenAI、Claude、Gemini ?
- **深度集成** - ?Python 数据科学/ML 技术栈无缝集成
- **RAG & Agent** - 内置检索增强生成和智能体工作流
- **高度可扩?* - 使用 Python 轻松定制和扩?
- **熟悉的生?* - 使用你喜爱的 Python 工具
...
| 功能 | 描述 |
|---------|-------------|
| **多提供商支持** | OpenAI、Claude、Gemini、Cohere、Azure OpenAI、DeepSeek、Groq、Mistral、Perplexity、Together AI、Fireworks AI ?[NVIDIA NIM](https://build.nvidia.com/) |
| **OpenAI 兼容 API** | 支持 15+ 个提供商?OpenAI 兼容接口 |
| **RAG 系统** | 混合向量搜索 (FAISS) + BM25 关键词检索，智能文档分割 |
| **函数调用** | 工具使用和外部命令执?|
| **AI 智能?* | 支持变量插值的多步骤工作流 |
| **交互?REPL** | 36 个内置命令，支持自动补全和语法高?|
| **会话管理** | 持久化对话，智能压缩 |
| **角色系统** | 可定制的 AI 人格，支持温?提示控制 |
| **HTTP 服务?* | 基于 FastAPI ?OpenAI 兼容 API 服务?|
| **视觉支持** | 多模态图像理解（支持视觉的模型） |

### 高级功能

- **流式响应** - 实时令牌流式传输
- **Markdown 渲染** - 美观的格式化输出和语法高?
- **代码执行** - 安全?Shell 命令执行
- **文件操作** - 在聊天中直接读取文件和目?
- **剪贴板集?* - 将响应复制到剪贴?
- **跨平?* - Windows、macOS、Linux
...
?`~/.config/aicortex/config.yaml` 创建配置文件?
...
# 使用角色
aicortex --role programmer "写一个斐波那契函?
...
| [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) | 项目结构和代码组?|
| [FEATURES.md](FEATURES.md) | ?Rust 版本的功能对照（91% 完成度） |
| [QUICKSTART.md](QUICKSTART.md) | 快速入门教?|
...
```bash
选项:
  -m, --model TEXT          使用的模?(? openai:gpt-4)
  --role TEXT               使用的角?(? programmer)
  -s, --session TEXT        会话名称
  --prompt TEXT             设置提示
  --rag TEXT                RAG 名称
  --agent TEXT              智能体名?
...
- **Windows**: `C:\Users\<用户?\.config\aicortex\config.yaml`
...
?`~/.config/aicortex/roles/` 创建自定?AI 人格?
...
# 使用 RAG 查询
aicortex --rag my-docs "文档中关?X 说了什么？"
```
...
1. Fork 本仓?
2. 创建你的特性分?(`git checkout -b feature/amazing-feature`)
3. 提交更改 (`git commit -m '添加某个很棒的功?`)
...
- MIT 许可?([LICENSE-MIT](LICENSE-MIT) ?https://opensource.org/licenses/MIT)
- Apache 许可?2.0 ?([LICENSE-APACHE](LICENSE-APACHE) ?https://www.apache.org/licenses/LICENSE-2.0)
...
本项目是 [sigoden](https://github.com/sigoden) 的原?[**AIChat**](https://github.com/sigoden/aichat) 项目?Python 重写版本，项目名称改?AICortex?
...
?Python 版本旨在提供功能对等，同时利?Python 生态系统实现更简单的定制和集成?
